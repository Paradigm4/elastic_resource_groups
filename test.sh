...
iquery -anq "create_with_residency(test_array_1, <val:double> [x=1:40,10,0], false, 'instances=0')" > test.out
iquery -anq "store(build(test_array_1, x), test_array_1)" >> test.out
iquery -anq "create array test_array_2 <val:double> [x=1:40,10,0]" >> test.out
